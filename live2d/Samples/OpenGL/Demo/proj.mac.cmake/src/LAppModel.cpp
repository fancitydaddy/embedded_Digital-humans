...
{
public:
    float GetAudioLevel(); // 获取音频电平值

private:
    std::vector<float> audioBuffer; // 用于存储音频数据的缓冲区

    void ProcessAudioData(); // 处理音频数据并计算RMS值
    float currentRms = 0.0f;
};

float AudioProcessor::GetAudioLevel()
{
    // 处理音频数据
    ProcessAudioData();
...
void AudioProcessor::ProcessAudioData()
{
    ...
    currentRms = sqrt(sum / audioBuffer.size());
}

// 获取音频电平值的示例函数
float GetAudioLevelExampleFunction()
{
    static AudioProcessor audioProcessor;
    return audioProcessor.GetAudioLevel();
}
namespace {
...
void LAppModel::Update()
{
    ...
    {
        // 获取最新的音频电平值
        csmFloat32 value = GetAudioLevelExampleFunction();
